...
async function createInputFiles() {
	const inputDayPath = path.join(startPath, "inputs", "2023", `day${day}`);
...
async function createCodeFiles() {
	const templateFolder = path.join(startPath, "src", "2023", "day0");
	const codeFolder = path.join(startPath, "src", "2023", `day${day}`);
...
async function updateIndex() {
	...
	const contents = (await promises.readFile(indexPath, { encoding: "utf8" }))
		.replace(
			"// MORE IMPORTS HERE",
			`import day${day} from './2023/day${day}/index';
// MORE IMPORTS HERE`
		)
		.replace(
			"// MORE DAYS HERE",
